...
int calculate_solar_panels(float load, float sunshine_hours) {
    float total_energy_required = load * 30 / sunshine_hours;  // Assuming 30 days in a month
    return static_cast<int>(total_energy_required / SOLAR_PANEL_CAPACITY);
}
...
float calculate_money_saved(float load, float sunshine_hours, float electricity_rate) {
    ...
    float energy_savings = energy_generated - (load * 30);  // Assuming 30 days in a month
    return energy_savings * electricity_rate * 12;  // Assuming 12 months in a year
}
...
int main() {
    ...
    case 1: {  // On-grid system
        ...
        cout << "Money saved per year: " << money_saved << " currency" << endl;
        break;
    }
